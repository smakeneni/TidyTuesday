...
USA_plot <- ggplot(USA_long_crops,aes(x=Year,y=cropquantity,fill=croptype))+geom_area(colour="black",alpha=0.6,size=0.5)+
  scale_fill_viridis(discrete = T,name="crop type")+theme_ipsum()+ggtitle("Crop Yeild in USA between 1961 and 2018")+
  scale_x_continuous(breaks=seq(1960,2020,5))+theme(axis.text.x=element_text(angle=45))+theme(plot.title = element_text(hjust=0.5))

#saving the plot
ggsave(plot=USA_plot,filename="USA_plot.png",device="png",dpi=300)
